      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			//calculate ref_v based on whether we are too close to the car in front or not
			for (int i=0; i<sensor_fusion.size();i++){
				...
				} else if (too_close && d<(4*lane) && d > (4*(lane-1))){
					if (check_speed < car_speed && (check_car_s < car_s - 10 || check_car_s > car_s + 40)){
						lane--;
						too_close = false;
					}
				} else if (too_close && d<(4*(lane+2)) && d > (4*lane+1)){
					if (check_speed < car_speed && (check_car_s < car_s - 10 || check_car_s > car_s + 40)){
						lane++;
						too_close = false;
					}
